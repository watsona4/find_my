...
TOPIC="find_my/healthcheck/status"
...
if ! pgrep -f "mosquitto_sub.*$TOPIC" > /dev/null; then
    ...
    mosquitto_sub -h "$BROKER" -t "$TOPIC" > /tmp/find_my_healthcheck_response.txt &
...
sleep "$TIMEOUT"
RESPONSE=$(tail -n 1 /tmp/find_my_healthcheck_response.txt)
